import { UPDATE_FEED, LIKE_POST, FeedActionTypes, PostInterface, FetchActionTypes } from '../types';
...
export function updateFeed() {
  return (dispatch: (arg0: FetchActionTypes) => void) => { // async action: uses Redux-Thunk middleware to return a function instead of an action creator
    dispatch(request());
...
export function likePost({ postId, userId }: { postId: String, userId: String }) {
  return (dispatch: (arg0: FetchActionTypes) => void) => {
    dispatch(request());
